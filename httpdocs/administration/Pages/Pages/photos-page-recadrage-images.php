...
$page_fonction = "?page=Photos-page"; //Page d'ou est appélé le script
...
$repertoire = "public_html/images/pages"; // répertoire d'upload
$repertoire_chmod = "../images/pages"; //répertoire du chmod
...
///////////////////////////////INSERT
$sql_insert = $bdd->prepare("INSERT INTO pages_a_b_image
	(id,
	img_lien,
	img_lien2,
	img_title,
...
alert('Il vous faut recadrer votre image, puis sauvegardez.');
return false;
};
</script>
...
if ($actionn == "recadrage"){
	...
	if($recadrage_active == "oui"){
		...
		$targ_w3 = intval($large_img_finale)-134;
		$targ_h3 = $haute_img_finale;
		$src3 = "".$repertoire_chmod."/2-home-".$_SESSION['namebruto_extension']."";
		$img_r3 = imagecreatefromjpeg($src);
		$dst_r3= ImageCreateTrueColor( $targ_w3, $targ_h3 );
		imagecopyresampled($dst_r3,$img_r3,0,0,$_POST['x']+100,$_POST['y'],$targ_w3,$targ_h3,$_POST['w']-200,$_POST['h']);
	...
	if($insert_sql == "oui"){
		$sql_insert->execute(array(
			...
			$_SESSION['attributs_title_alt_image'],
			$_SESSION['id_page_photo'],
			''));                     
		$sql_insert->closeCursor();
	}

	chmod("$repertoire_chmod/2-home-".$_SESSION['namebruto_extension'], 0777);
	chmod("$repertoire_chmod/2-".$_SESSION['namebruto_extension'], 0777);
...
if($actionn == "upload"){
       ...
       $icon = $_FILES['images']['name'];
		...
		$erreur_upload = "Une erreur est survenue, merci de tenter de nouveau le téléchargement !";
		}
		////////////Si n'est pas une image ou si aucune image choisie
...
////////////////////////////////////////UPLOAD FORMULAIRE
	...
	?>
			<span class='rapport' style='color: green;'>Votre image a été téléchargée avec succès !</span><br /><br />
