...
if (isset($user)) {
    ...
    $req_boucle->execute(array($idColis));
    while ($ligne_boucle = $req_boucle->fetch()) {
        ...
        $sql_update->execute(
            array(
                ...
                $_POST['description'.$ligne_boucle['id']],
                $_POST['categorie'.$ligne_boucle['id']],
                $_POST['quantite'.$ligne_boucle['id']],
                $_POST['type_value'.$ligne_boucle['id']],
                $_POST['value'.$ligne_boucle['id']],
    ...
    $in = false;
   /* $req_boucle = $bdd->prepare("SELECT * FROM membres_colis_details WHERE colis_id=? AND (annule != 'oui' OR annule is null) AND (disponibilite != 'Non disponible' OR disponibilite is null) ORDER BY id DESC");
    $req_boucle->execute(array($idColis));
    while ($ligne_boucle = $req_boucle->fetch()) {
        $in = true;
        ...
        ///////////////////////////////Douane et transport colis normale
        ...
        $prix_expedition = (($ligne_boucle['quantite']*$ligne_boucle['prix_reel'])*($categorie['value_colis']/100)); //$ligne_boucle['quantite']*
    ...
    $req_select = $bdd->prepare("SELECT * FROM membres_colis WHERE id=?");
    $req_select->execute(array($idColis));
