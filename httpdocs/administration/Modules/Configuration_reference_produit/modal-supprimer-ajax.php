...
if (
  ...
  // Fetch product info to display in the confirmation modal
  $req_select = $bdd->prepare("SELECT nom_produit, ref_produit_hyro FROM configurations_references_produits WHERE id=?");
  $req_select->execute(array($idaction));
  $ligne_select = $req_select->fetch();
  $req_select->closeCursor();
  
  $nom_produit = $ligne_select['nom_produit'];
  $ref_produit = $ligne_select['ref_produit_hyro'];
?>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <?php if (!empty($nom_produit)) { ?>
            <div class="alert alert-warning">
              <strong>Produit :</strong> <?php echo $nom_produit; ?><br>
              <strong>Référence :</strong> <?php echo $ref_produit; ?>
            </div>
          <?php } ?>
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-danger" id="btnSuppr" data-id="<?php echo $idaction; ?>">Oui, supprimer</button>
        </div>
      </div>
    </div>
  </div>
